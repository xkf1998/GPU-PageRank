...
void pagerank(const int nodes, const int edges, float* value, const int* rowdeg, const int* colptr, const int* row, const int* col, const int iteration)
{
...
{   
    if (argc != 7 || strcasecmp("-f", argv[1]) || strcasecmp("-o", argv[3]) || strcasecmp("-i", argv[5])) {
        fprintf(stderr, "Usage ./pagerank -f (file name) -o (output file name) -i (iterations)\n");
        return 1;
    }
    ...
    pagerank(nodes, edges, value, rowdeg, colptr, row, col, iteration);
